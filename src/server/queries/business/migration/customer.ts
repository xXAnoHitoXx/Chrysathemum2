...
export const migrate_customer_data: Query<OldCustomerData, Customer> = async (
    data,
    f_db,
) => {
    const context = "Migrating customer { ".concat(
        data.name,
        "-",
        data.phoneNumber,
        " }",
    );
    const result = await retrieve_customer_id_from_legacy_id(
        { legacy_id: data.id },
        f_db,
    );
...
export const import_customer_from_old_db: Query<
    void,
    PartialResult<OldCustomerData[]>
> = async (_, f_db) => {
    const context = "Import Customers from Old DB";
    const data_snapshot = await db_query(
        context,
        get(f_db.old_db(["customer", "id"])),
    );
