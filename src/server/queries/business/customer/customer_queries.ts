...
export const create_new_customer: Query<CustomerCreationInfo, Customer> =
    async (data, f_db) => {
...
export const update_customer_info: Query<{ customer: Customer, update: CustomerUpdateData }, Customer> =
    async ({customer, update}, f_db) => {
        ...
        if(is_data_error(del_phone_index)) return del_phone_index.stack(context, "...");
        ...
        if(is_data_error(update_customer)) return update_customer.stack(context, "...");

        const create_phone_index = await create_customer_phone_index(update_target, f_db);
        if(is_data_error(create_phone_index)) return create_phone_index.stack(context, "...");
