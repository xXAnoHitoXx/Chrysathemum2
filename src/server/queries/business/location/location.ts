...
export const retrieve_technicians_at_location: Query<{ location_id: string }, PartialResult<Technician[]>> =
    async (data, f_db) => {
        const context = "Retrieve Technicians at { ".concat(data.location_id, " }");
        const v: void = undefined;
        const roster = await retrieve_roster(data, f_db);
        if(is_data_error(roster)) { return roster.stack(context, "...") }

        const technicians = await get_all_technicians(v, f_db);
        if(is_data_error(technicians)) { return technicians.stack(context, "...") }
        ...
        const technicians_at_location = technicians.data.filter((t: Technician) => (
