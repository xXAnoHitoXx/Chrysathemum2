...
export const assign_technician_to_roster: 
    Query<
        { location_id: string, technician: { technician_id: string, color: string } }, void
    > = async ( { location_id, technician }, f_db ) => {
        return db_query(
            "assigning technician { ".concat( technician.technician_id, " } to { ", location_id, " }"),
            set(
...
> = async ({ location_id }, f_db) => {
    ...
    const data = await db_query(context, get( f_db.location_roster([ location_id ])));
...
export const remove_technician_from_roster: Query< { location_id: string, technician_id: string }, void > = 
    async (  { location_id, technician_id }, f_db) => {
        return db_query(
            "Removing technician { ".concat(technician_id, " } from roster { ", location_id, " }"),
