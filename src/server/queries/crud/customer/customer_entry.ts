...
export const create_customer_entry: Query<
...
export const retrieve_customer_entry: Query<
    { customer_id: string },
    Customer
> = async ({ customer_id }, f_db) => {
    ...
    const data = await db_query(
        context,
        get(f_db.customer_entries([customer_id])),
    );
...
export const update_customer_entry: Query<Customer, void> = async (
    customer,
    f_db,
) => {
    return db_query(
...
export const delete_customer_entry: Query<{ id: string }, void> = async (
    { id },
    f_db,
) => {
    return db_query(
