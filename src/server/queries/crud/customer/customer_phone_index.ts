...
export const create_customer_phone_index: Query<Customer, void> = async (
    customer,
    f_db,
) => {
    return db_query(
        ...
        set(
            f_db.customers_phone_index([customer.phone_number, customer.id]),
            customer.id,
        ),
    );
};

export const retrieve_customer_phone_index: Query<
    { phone_number: string },
    PartialResult<{ customer_ids: string[] }>
> = async ({ phone_number }, f_db) => {
    const context = "Retrieving customers with phone number { ".concat(
        phone_number,
        " }",
    );
    const index: string[] = [];
    ...
    const data = await db_query(
        context,
        get(f_db.customers_phone_index([phone_number])),
    );
    ...
    return {
        ...
        error:
            error.length == 0
                ...
                : lotta_errors(
                      context,
                      "index contained some corrupted entries",
                      error,
                  ),
    };
};

export const delete_customer_phone_index: Query<Customer, void> = async (
    customer,
    f_db,
) => {
    return db_query(
