...
export async function create_phone_index(customer: Customer, redirect: string) {
    await set(ref(f_db, fb_customers_phone_index(redirect).concat(customer.phone_number, "/", customer.id)), customer.id);
}

export async function retrieve_phone_index(phone_number: string, redirect: string): Promise<string[]> {
    const index: string[] = [];
...
export async function delete_phone_index(customer: Customer, redirect: string) {
