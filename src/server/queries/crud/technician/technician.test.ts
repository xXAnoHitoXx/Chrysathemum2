...
import {
    create_technician_login_index,
    delete_technician_login_index,
    retrieve_technician_id_from_user_id,
} from "./technician_login_index";
import {
    create_technician_migration_index,
    delete_technician_migration_index,
    retrieve_technician_id_from_legacy_id,
} from "./technician_migration_index";
...
test("test technician entries CRUD queries", async () => {
    ...
    const test_technician_entry = await pack_test(
        { name: "Tinn", color: "blu", active: false },
        test_name,
    )
    ...
    const created_technician_entry = await pack_test(
    ...
    const updated_technician_entry = await pack_test(
        { id: test_technician_entry.id },
        test_name,
    )
        .bind(retrieve_technician_entry)
    ...
    const no_technician_entry = await pack_test(
        { id: test_technician_entry.id },
        test_name,
    )
        .bind(retrieve_technician_entry)
...
test("test technician_login_index CRUDs querries", async () => {
    ...
    let conversion = await pack_test({ user_id: login.user_id }, test_name)
    ...
    conversion = await pack_test({ user_id: login.user_id }, test_name)
        .bind(retrieve_technician_id_from_user_id)
        .unpack();
...
test("test technician_migration_index CRUDs querries", async () => {
    const test_name = test_suit.concat(
        "/test_technician_migration_index_cruds/",
    );
    ...
    conversion = await pack_test({ legacy_id: migration.legacy_id }, test_name)
        .bind(retrieve_technician_id_from_legacy_id)
        .unpack();
