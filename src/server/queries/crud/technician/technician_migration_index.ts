...
export const create_technician_migration_index: Query<{ legacy_id: string, technician_id: string },void> =
    async ({ legacy_id, technician_id }, f_db) => {
        ...
        return db_query("Creating Technician Migration Index", set(ref, technician_id))
    }

export const retrieve_technician_id_from_legacy_id: Query<{ legacy_id: string }, { technician_id: string }> =
    async ({ legacy_id }, f_db) => {
        const context = "Retrieve Technician { ".concat(legacy_id, " } from Migration Index");

        const data = await db_query(context, get(f_db.technician_legacy_index([legacy_id])));
...
export const delete_technician_migration_index: Query<{ legacy_id: string }, void> = 
    async ({ legacy_id }, f_db) => {
        return db_query("Deleting Migration Index", remove(f_db.technician_legacy_index([legacy_id])));
    }
