...
> = async (_, f_db) => {
    ...
    const data = await db_query(
        context,
        get(f_db.technician_legacy_index([])),
    );
    ...
    if (data.exists()) {
        data.forEach((child) => {
            ...
            if(is_string(tech_id)) {
                ...
                errors.push(data_error("retrieve index", `corrupted index ${child.key}`))
            }
        });
    }

    return { data: index, 
        error: (errors.length == 0)? null : lotta_errors(context,"error retrieving some index", errors) };
}
