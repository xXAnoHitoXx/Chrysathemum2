...
> = async ({ customer_id }, f_db) => {
    ...
    const transactions: { id: string; date: string; salon: string }[] = [];
    const error: DataError[] = [];

    data.forEach((child) => {
        const { date, salon } = child.val();
        if (is_string(date) && is_string(salon)) {
            transactions.push({ id: child.key, date: date, salon: salon });
        } else {
            error.push(
