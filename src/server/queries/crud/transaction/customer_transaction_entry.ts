...
export const create_customer_trasaction_history_entry: Query<{ customer_id:string, id:string, date: string }, void> = 
...
export const retrieve_customer_transactions_history
    : Query<{ customer_id: string }, PartialResult<{ id: string, date: string }[]>> = 
    async ({ customer_id }, f_db) => {
        ...
        data.forEach((child) => {
            ...
            if(is_string(date)){
                ...
                error.push(data_error(context, child.key.concat(" value is not a string")));
            }
        });
...
export const delete_customers_transaction_history_entry : Query<{ customer_id: string, id: string }, void> =
