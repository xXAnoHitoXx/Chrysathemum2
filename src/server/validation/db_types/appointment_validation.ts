...
export function to_appointment(t: unknown): AppointmentEntry | TypeConversionError {
    if (!is_object(t)) {
        return { error: "unknown is not AppointmentEntry" };
    }
    ...
    if (time < 1 || time > 52) {
        return { error: "AppointmentEntry time needs to be between 1-52" };
    }
