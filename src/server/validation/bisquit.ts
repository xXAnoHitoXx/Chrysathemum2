...
export function to_bisquit(u: unknown): Bisquit | DataError {
    if (!is_string(u)) return data_error("Casting to Bisquit", "not a string");

    if (!is_bisquit(u))
        return data_error(
            "Casting to Bisquit",
            "no Bisquit associated with string u",
        );

    return Bisquit[u];
}

export function to_bisquit_data(
    u: unknown,
): { name: Bisquit; value: string } | DataError {
    if (!is_object(u))
