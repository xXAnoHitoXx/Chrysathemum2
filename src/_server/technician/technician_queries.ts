...
export class TechnicianQuery {
    ...
    > = ServerQuery.from_builder((info: TechnicianCreationInfo) =>
        create_technician_entry.chain<[TechnicianCreationInfo, Technician]>(
            (technician: Technician) => [info, technician],
        ),
    ).chain<Technician>(
        ServerQuery.from_builder(([info, technician]) =>
